...
def train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader):
    ...
    return training_loss/total_batchs

    
def plot_learning_curve(total_loss, ep):
    plt.plot(range(1, ep+1), total_loss)
    plt.title("Learning Curve")
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.savefig('./records/CUB200-T2/T3000/learning_curve.png')
...
def main(args, tlogger):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if train_loader is not None:
            ...
            loss_tmp = train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader)
            total_loss.append(loss_tmp.item())
            tlogger.print()
        else:
    ...
    if train_loader is not None:
        plot_learning_curve(total_loss, (args.max_epochs - start_epoch))
